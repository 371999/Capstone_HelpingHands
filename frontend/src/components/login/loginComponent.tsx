...
export default function LoginComponennt() {
  ...
  const [registrationFormValidation, setRegistrationFormValidation] =
    useState<string>("");
  ...
  const registrationHandler = async (e: FormEvent) => {
    ...
    if (
      (profileType === "gym" && !gymName) ||
      (profileType === "user" && (!firstName || !lastName)) ||
      !email ||
      !password ||
      !confirmPassword
    ) {
    ...
    if (!emailRegex.test(email)) {
      setRegistrationFormValidation(
        "Email Address format is invalid. Try Again!"
      );
      return;
    }
    ...
    if (!strongPasswordRegex.test(password)) {
      setRegistrationFormValidation(
        "Password should length should have 8 characters including special character and number"
      );
      return;
    }
