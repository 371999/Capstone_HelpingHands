...
export default function Component() {
  ...
  const fetchUserRequestDetails = async () => {
    try {
      const response = await httpFetch(
        `${GET_REQUEST_FOR_USER}/${itemDetails._id}/${user.id}`
      );
      const data = await response.json();
    ...
    try {
      ...
      if (response.ok) {
        giveReviewToast(
          "success",
          `${
            requestType === "delivery" ? "Delivery" : "Pick Up"
  ...
  const handleDeleteRequest = async () => {
    try {
      setRequestProcessing(true);
      const response = await httpFetch(`${DELETE_REQUEST}/${userRequest._id}`, {
        method: "DELETE",
      });
      if (response.ok) {
            ...
            userRequest.status === 1 ? (
              ...
              <>
               <div className="mt-4">
                ...
                <Button
                  className="w-full"
                  onClick={() => handleCreateRequest("delivery")}
                  disabled={requestProcessing}
                >
                  Request Delivery
                </Button>
                <Button
                  ...
                  onClick={() => handleCreateRequest("pick up")}
                  disabled={requestProcessing}
                >
                  Request Pick Up
                </Button>
